/**
 * Visibility reconstruction helpers for software raster shading.
 */
#pragma once
...
float3 ReconstructWorldPos(uint2 pixel, float depth, uint width, uint height, float4x4 clip2world) {
    float2 screen_uv = (float2(pixel) + 0.5) / float2(width, height);
    float2 ndc = screen_uv * 2.0 - 1.0;
    ndc.y = -ndc.y;

    float4 clip = float4(ndc, depth, 1.0);
    float4 world_h = mul(clip2world, clip);
